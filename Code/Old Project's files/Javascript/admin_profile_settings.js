document.addEventListener("DOMContentLoaded", function () {
  ...
  changePasswordForm.addEventListener("submit", function (event) {
    ...
    if (!passwordPattern.test(new_password)) {
      showMessage(
        "error-message",
        "Ο νέος κωδικός πρέπει να είναι τουλάχιστον 8 χαρακτήρες και να περιέχει τουλάχιστον ένα κεφαλαίο γράμμα, ένα αριθμό και ένα σύμβολο (π.χ. #$*&@)",
        "newPassword"
      );
      return;
      ...
      showMessage(
        "error-message",
        "O καινουργιος κωδικός δεν μπορει να είναι ιδιος με τον παλιό.",
        "newPassword"
      );
      return;
    } else {
...
function GetAdminInfo() {
  ...
  $.ajax({
    url: "https://localhost/webproject/Code/PHP/get_admin_data.php", // Αντικαταστήστε με τον σωστό δρόμο προς το PHP αρχείο
    method: "GET",
...
function AdminChangePassword(oldPassword, newPassword) {
  $.ajax({
    url: "https://localhost/webproject/Code/PHP/Change_Password.php", // Αντικαταστήστε με τον σωστό δρόμο προς το PHP αρχείο
    method: "POST",

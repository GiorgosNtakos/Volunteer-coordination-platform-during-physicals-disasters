...
"use strict";
 ...
 function signup(event) {
  ...
  if (username.trim() === '') {
    showMessage("error-message", "Συμπληρώστε το πεδίο Username.", "user-signup");
    return;
  }

  if (password.trim() === '') {
    showMessage("error-message", "Συμπληρώστε το πεδίο Password.", "pass-signup");
    return;
  }

  if (repeatPassword.trim() === '') {
    showMessage("error-message", "Συμπληρώστε το πεδίο Repeat Password.", "pass-repeat-signup");
    return;
  }

  if (email.trim() === '') {
    showMessage("error-message", "Συμπληρώστε το πεδίο Email Address.", "email-signup");
    return;
  }

  // Ελέγχουμε αν οι κωδικοί είναι ίδιοι
  if (password !== repeatPassword) {
    showMessage("error-message", "Οι κωδικοί δεν ταιριάζουν. Παρακαλώ δοκιμάστε ξανά.", "pass-repeat-signup");
    return;
  }
  ...
  if (!passwordPattern.test(password)) {
    showMessage("error-message","Ο κωδικός πρέπει να είναι τουλάχιστον 8 χαρακτήρες και να περιέχει τουλάχιστον ένα κεφαλαίο γράμμα, ένα αριθμό και ένα σύμβολο (π.χ. #$*&@).","pass-signup");
    return;
  }
    ...
    if (!emailPattern.test(email)) {
      showMessage("error-message","Παρακαλώ εισάγετε ένα έγκυρο email(π.χ. user@domain.com).", "email-signup");
      return;
    }
  ...
  $.ajax({
    url: "http://localhost/webproject/Code/PHP/sign_up.php",
    method: "POST",
    ...
    success: function(response) {
      ...
      if ( response.status === "created") {
        ...
        showMessage("success-message", response.message, "user-signup");
        
        // Ενέργειες που θέλετε να κάνετε μετά την επιτυχημένη εγγραφή
      } else {
        // Η εγγραφή απέτυχε
        showMessage("error-message", "Ενώ η εγγραφή ήταν επιτυχής κάτι πήγε στραβά.Παρακάκω δοκιμάστε ξανά" , "user-signup");
    ...
    error: function(response) {
      ...
      var errorResponse = JSON.parse(response.responseText);
          if (errorResponse.status === "exists_name") {
                ...
                showMessage("error-message", errorResponse.message, "user-signup");
                
            } else if(errorResponse.status === "exists_email"){
              
              showMessage("error-message", errorResponse.message, "email-signup");
              ...
              showMessage("error-message", errorResponse.message, "user-signup");
                ...
                showMessage("error-message", "Σφάλμα κατά την εγγραφή. Παρακαλώ δοκιμάστε ξανά.", "user-signup");
            }
